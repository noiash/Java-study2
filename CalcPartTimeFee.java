...
public class CalcPartTimeFee {
	...
	public static void main(String[] args) {
		...
		final int CONV_HOUR_TO_MILLSEC = 1000 * 60 * 60;						//時間のミリ秒換算
		final int CONV_MINUTE_TO_MILLSEC = 1000 * 60;							//分のミリ秒換算
		final int CONV_HOUR_TO_MINUTE = 60;										//時間の分換算
		final long WORK_TIME_TYPE1_END = 6 * CONV_HOUR_TO_MILLSEC;				//労働時間タイプ①（ミリ秒）（労働時間〜6時間以下）ー終了時間
		final long WORK_TIME_TYPE2_START = 6 * CONV_HOUR_TO_MILLSEC;			//労働時間タイプ②（ミリ秒）（労働時間6時間超〜8時間以下）ー開始時間
		final long WORK_TIME_TYPE2_END = 8 * CONV_HOUR_TO_MILLSEC;				//労働時間タイプ②（ミリ秒）（労働時間6時間超〜8時間以下）ー終了時間
		final long WORK_TIME_TYPE3_START = 8 * CONV_HOUR_TO_MILLSEC;			//労働時間タイプ③（ミリ秒）（労働時間8時間超〜）開始時間
		final long REST_TIME_TYPE1 = 45 * CONV_MINUTE_TO_MILLSEC;				//休憩時間（ミリ秒）＊労働時間タイプ②に適用
		final long REST_TIME_TYPE2 = 60 * CONV_MINUTE_TO_MILLSEC;				//休憩時間（ミリ秒）＊労働時間タイプ③に適用
		...
		if (workTime <= WORK_TIME_TYPE1_END) {
			...
			//実労働時間（分）の計算
			actualWorkTimeMin = (int)(workTime / CONV_MINUTE_TO_MILLSEC);
			
		} else if (workTime > WORK_TIME_TYPE2_START && workTime <= WORK_TIME_TYPE2_END) {
			//労働時間タイプ②　（労働時間6時間超〜8時間以下）の場合
			actualWorkTimeMin = (int)((workTime - REST_TIME_TYPE1) / CONV_MINUTE_TO_MILLSEC);
			
		} else if (workTime > WORK_TIME_TYPE3_START) {
			//労働時間タイプ③　（労働時間8時間超〜）
			
			//実労働時間（分）の計算
			actualWorkTimeMin = (int)((workTime - REST_TIME_TYPE2) / CONV_MINUTE_TO_MILLSEC);
		}
